...
fn part1(input: &str) -> anyhow::Result<i64> {
    ...
    let result = records.iter().map(|r| count_arrangements(&r.spring_states, 0, &r.group_counts)).sum();
    Ok(result)
}
...
fn count_arrangements2(spring_states: &[SpringState], group_counts: &[i64]) -> i64 {
    ...
    let result = count_arrangements(&s, 0, &g);
    println!("...{result}");
    result
}

fn count_arrangements(spring_states: &[SpringState], current_count: i64, group_counts: &[i64]) -> i64 {
    ...
    if spring_states.is_empty() {
        return if current_count == first_count {
            count_arrangements(spring_states, 0, &group_counts[1..])
        } else {
            // println!("FALSE!");
            0
        };
    ...
    let result = match first_state {
        SpringState::Operational => {
            if current_count == first_count {
                count_arrangements(&spring_states[1..], 0, &group_counts[1..])
            } else if current_count > 0 {
                // println!("FALSE!");
                0
            } else {
                count_arrangements(&spring_states[1..], 0, group_counts)
            }
        }
        SpringState::Damaged => {
            count_arrangements(&spring_states[1..], current_count + 1, group_counts)
        }
        SpringState::Unknown => {
            let damaged = count_arrangements(&spring_states[1..], current_count + 1, group_counts);
...
?###???????? 3,2,1"#;
    ...
    fn test1() -> anyhow::Result<()> {
        ...
        let result = count_arrangements(&record.spring_states, 0, &record.group_counts);
        let expected = 1;
