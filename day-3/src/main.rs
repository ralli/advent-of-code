...
fn part1(input: &str) -> u32 {
    let numbers = input.lines().map(|l| l.chars().collect::<Vec<_>>()).collect::<Vec<_>>();
    ...
    let e = gamma.iter().map(|&c| if c == '1' { 0 } else { 1 }).fold(0, |n, d| 2 * n + d);
...
fn part2(input: &str) -> u32 {
    ...
    for i in 0..len {
        if oxigen_rating.len() > 1 {
            ...
            oxigen_rating = oxigen_rating.into_iter().filter(|v| v[i] == mbit).collect::<Vec<_>>();
        }
        if co2_rating.len() > 1 {
            let mbit = most_common_bit(&co2_rating, i);
            co2_rating = co2_rating.into_iter().filter(|v| v[i] != mbit).collect::<Vec<_>>();
        }
    }
...
fn to_binary(v: &[char]) -> u32 {
    v.iter().map(|&c| if c == '1' { 1 } else { 0 }).fold(0, |n, d| 2 * n + d)
}
